...
const log = util.debuglog('git.js');
...
function getCacheDir(optPath) {
    const dir = findCacheDir({name: 'master'});
...
exports.defaults = {
    ...
    push: true,
    history: true,
    message: '👨‍💻CODE BY 🕊️★⃝AJAY O S©️🧚‍♂️',
    silent: false,
};
 ...
 */
 exports.publish = function publish(basePath, config, callback) {
    ...
    if (!callback) {callback = function (err) {if (err) {log(err.message);};};};

    function done(err) {try {callback(err);} catch (err2) {log('Publish callback threw: %s', err2.message);}};

    try {if (!fs.statSync(basePath).isDirectory()) {done(new Error('The "base" option must be an existing directory'));return;};} catch (err) {done(err);return;};

    const files = globby.sync(options.src, {cwd: basePath,dot: options.dotfiles,}).filter((file) => {return !fs.statSync(path.join(basePath, file)).isDirectory();});

    if (!Array.isArray(files) || files.length === 0) {done(new Error('The pattern in the "src" property didn\'t match any files.'));return;}

    let repoUrl;
    let userPromise;
    if (options.user) {userPromise = Promise.resolve(options.user);} else {userPromise = getUser();};
    ...
    .then((repo) => {
        ...
        return git.getRemoteUrl(options.remote).then((url) => {
            if (url !== repoUrl) {throw new Error('Remote url mismatch.  Got "' + url + '" ' + 'but expected "' + repoUrl + '" in ' + git.cwd);}
            return git;
        });
    }).then((git) => {
        log('Cleaning');
        ...
        if (!options.history) {return git.deleteRef(options.branch);} else {return git;};
    }).then((git) => {
        if (options.add) {return git;};
        log('Removing files');
        const files = globby.sync(options.remove, {cwd: path.join(git.cwd, options.dest),}).map((file) => path.join(options.dest, file));
        if (files.length > 0) {return git.rm(files);} else {return git;};
    }).then((git) => {
        log('Copying files');
        return copy(files, basePath, path.join(git.cwd, options.dest)).then(function () {return git;});
    }).then((git) => {
...
exports.clean = function clean() {fs.removeSync(getCacheDir());};
